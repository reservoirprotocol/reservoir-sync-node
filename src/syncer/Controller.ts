...
export class Controller {
  ...
  private async _getInitialBlock(contract?: string): Promise<Block | null> {
    ...
    return {
      id: v4(),
      priority: 1,
      startDate: reqs[0].data[root][0].updatedAt,
      endDate: reqs[1].data[root][0].updatedAt,
      contract: contract || "",
    };
  }
  /**
  ...
  public normalize(
    ...
    if (params?.contract) {
      if (this._config.dataset == "bids" || this._config.dataset == "asks") {
        params["contracts"] = params.contract;
