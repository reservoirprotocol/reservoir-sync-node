...
class SyncNode {
  ...
  /**
   *
   * @param contract Adds a contract to the sync service
   */
  public async addContract(contract: string, type: DataTypes): Promise<void> {
    // Update the contracts that are in redis
    await this._queueService.updateContracts(
      [contract, ...this._contracts],
      type
    );

    this._controllers.get(type)?.addContract(contract);
  }
